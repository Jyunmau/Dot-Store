...
def get_report_summary(shop_id: int, date: str = None, db: Session = Depends(get_db)):
    """获取报表汇总"""
    try:
        # 构建查询条件
...
def get_income_structure(shop_id: int, date: str = None, db: Session = Depends(get_db)):
    """获取收入结构"""
    try:
        # 构建查询条件
...
def get_expense_structure(shop_id: int, date: str = None, db: Session = Depends(get_db)):
    """获取成本结构"""
    try:
        # 构建查询条件
        query = db.query(LedgerEntry.account_id, func.sum(LedgerEntry.amount))
